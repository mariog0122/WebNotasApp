      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 flex-1 min-w-0">
        ...
        <div class="min-w-0">
          <label class="block text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1 flex items-center gap-1.5 truncate">
            <School class="w-3.5 h-3.5 text-teal-600 shrink-0" />
            <span>Curso:</span>
          </label>
          <select
            :value="gridCourseId"
            @change="onCourseChange($event.target.value)"
            class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2.5 text-xs font-semibold text-slate-900 dark:text-white focus:ring-2 focus:ring-teal-500 focus:outline-none min-w-0 truncate"
            aria-label="Seleccionar curso para sábana mensual"
          >
        ...
        <div class="min-w-0">
          ...
          <input
            ...
            aria-label="Seleccionar mes para sábana mensual"
          />
        </div>
      </div>

      <!-- Export & Print Actions -->
      <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 pt-2 md:pt-0 border-t md:border-t-0 border-slate-100 dark:border-slate-800 shrink-0">
        <button
          type="button"
          @click="handleExportCsv"
          class="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-3.5 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-xs font-bold text-slate-700 dark:text-slate-300 transition-colors cursor-pointer min-h-[40px]"
        >
        ...
        <button
          type="button"
          @click="handlePrint"
          class="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl bg-teal-600 hover:bg-teal-700 text-white text-xs font-bold shadow-sm transition-colors cursor-pointer min-h-[40px]"
        >
